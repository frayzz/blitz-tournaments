        ...
        <div class="relative flex items-top justify-center min-h-screen pt-10 py-4 sm:pt-0">
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                    @auth
                        <a href="{{ url('/dashboard') }}" class="text-sm bg-black color-white underline">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="btn btn-info">Войти</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="btn btn-secondary">Зарегестрироваться</a>
                        @endif
                    @endauth
                </div>
            @endif

            <div class="mx-auto sm:px-6 lg:px-8 cr-white w-100">
...
<style>
    .tournament_styles tbody tr td, th {
